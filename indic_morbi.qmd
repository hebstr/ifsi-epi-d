...
La [morbidité]{.cb} est relative au fait d'être [atteint d'une maladie]{.cb}.
...
\mathrm{
\frac
{Nombre \ de \ nouveaux \ cas \ survenus \ pendant \ une \ période}
...
L'incidence reflète la [vitesse]{.cb} de survenue d'une maladie dans une population.
...
C'est la proportion de [personnes atteintes d'une maladie à un instant donné]{.cb} dans la population.
...
L'incidence, la prévalence et la relation qui existe entre elles peuvent être illustrées avec [la baignoire de l'épidémiologiste]{.cb}.
...
La mort n'est pas la seule manière de ne plus être malade : il y a aussi la possibilité d'une [guérison]{.cb}.
Ce 4e élément peut être symbolisé par l'évaporation.
...
- Un volume [entrant]{.cb} : l'apparition de nouveaux cas = [incidence]{.cb}
- Un volume [constitué]{.cb} : le nombre de cas constitués = [prévalence]{.cb}
- Un volume [sortant]{.cb} : la [mort]{.cb} ou la [guérison]{.cb}

Ainsi, bien que le robinet soit constamment ouvert, la baignoire ne déborde pas. En revanche, s'il y a une limite à la sortie, le volume d'eau aura tendance à augmenter avec le temps.
